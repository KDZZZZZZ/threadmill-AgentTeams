...
func TestProjectionReadTaskScansInterleavedEventsAndFailsClosed(t *testing.T) {
	t.Parallel()
	...
	if _, _, err := projection.ReadTask(ctx, Principal{Role: RoleAuditor, ProjectID: "project-2", TaskID: "task-1"}, "task-1", 0, 1); !kernel.IsCode(err, kernel.CodeForbidden) {
		t.Fatalf("cross-project projection error = %v", err)
	}
