...
func physicalRecord(epoch ExecutionEpoch, task string) PhysicalExecution {
	return PhysicalExecution{TaskID: "task-a", InvocationID: "inv-a", Generation: 3, ExecutionEpoch: epoch, WorkerID: "worker-" + task, WorkerName: "worker-" + task, TeamHarnessTaskID: task, MCPClientID: "threadmill", CredentialBindingRef: "credential-" + task, ExecutionAuthorizationRef: "auth-" + task, WorkspaceLeaseRef: "lease-" + task, DesiredRuntimeGeneration: 9, AppliedRuntimeGeneration: 9, State: PhysicalExecutionAccepted, ObservedTaskStatus: "in_progress"}
}
...
func TestValidatePhysicalExecutionReady(t *testing.T) {
	...
	for name, mutate := range map[string]func(*PhysicalExecution){"worker": func(e *PhysicalExecution) { e.WorkerID = "" }, "task": func(e *PhysicalExecution) { e.TeamHarnessTaskID = "" }, "generation": func(e *PhysicalExecution) { e.AppliedRuntimeGeneration-- }, "mcp": func(e *PhysicalExecution) { e.MCPClientID = "" }, "acceptance": func(e *PhysicalExecution) { e.ObservedTaskStatus = "assigned"; e.TaskAcknowledged = false }} {
