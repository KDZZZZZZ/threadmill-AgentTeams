...
`runtime.awaitInputs` is now connected through the HTTP MCP transport to a binding-scoped Runtime coordinator. The coordinator accepts only input IDs already present in the authoritative pending set; it stores no token in continuation or waiting records. Input arrival is represented by a new immutable complete `PhaseInputSet`, from which reconstruction computes `NewlyDelivered` and a fresh BindingRef. This is await continuation within one logical Invocation/Generation, not checkpoint resume.
