...
M5-C4-3 adds a strictly read-only restart observation seam. Runtime first loads its durable `RecoverySnapshot` and acquires a fenced `RecoveryClaim`; only then may the AgentTeams adapter read Controller `GET /api/v1/workers/{name}/status` and TeamHarness `check_task`. The resulting Worker/task/runtime/MCP view is execution-plane evidence, not Runtime truth: a ready Worker cannot terminalize or resume an invocation, a submitted TeamHarness task cannot become a `PhaseOutput`, and a Matrix room cannot restore a QwenPaw/provider conversation.

The observer validates the durable epoch-encoded Worker/task names, redacted desired/applied runtime generation, MCP client application and safe room reference. A 404 is `not_found`; timeout, connection failure and 5xx remain `unknown/unavailable`. Before returning the observation, Runtime asserts the claim and snapshot fingerprint again, discarding an observation if another actor changed durable state. C4-3 never deletes/recreates a Worker, signs a token, changes an epoch, delegates a task, applies runtime configuration or writes an outbox event; a later recovery-decision slice owns those actions.

## Lost-carrier fencing

M5-C4-4 consumes only a claim-fenced, non-stale observation together with the Runtime snapshot. A missing/failed Worker (or a durable failed physical record) can fence the old epoch and reserve a fresh one; a ready verified Worker, unknown/timeout, generation pending, MCP pending and normal restart cannot. Identity mismatch and a terminating Worker are not treated as healthy, but are held for reconciliation instead of creating overlapping carriers.

The Runtime repository, not Controller naming or a process-local counter, allocates the replacement epoch from the maximum durable physical history. Its transaction fences the old record, reserves a successor in reserved state, changes Waiting from running to rehydrating, and records redacted outbox evidence. The reservation contains logical identity, BindingRef/InputRevision and opaque continuation/evidence refs only. It does not create a Worker/task/token/credential/lease/session or reuse the old receipt; C4-5 must provision a new physical carrier and obtain a fresh package-consumption receipt. Artifact ownership remains logical TaskID+InvocationID, so prior refs remain usable without implying a successful PhaseOutput.
