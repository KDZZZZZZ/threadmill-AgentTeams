...
The M4-E3 extension continues in that same fresh session: the deterministic provider registers a controlled artifact and invokes `agent.submitPhaseOutput` with its real `ArtifactRef`. The fixture observes the durable output/event, logical `terminal` state, physical `terminated` evidence, exact token-B revocation and conflict-path reclamation. It never converts TeamHarness acknowledgement/submission into PhaseOutput and never restores an old QwenPaw/provider session.

M4-E4 extends the same fixture rather than adding another stack. It first creates a real Worker-A/task-A and lets its QwenPaw session call `runtime.awaitInputs`. After authoritative input revision r5 is stored, the production reconstruction and provisioning seams create Worker-B/task-B and a different Matrix session. The fixture verifies both carrier histories, B1-to-B2 rebinding, token/credential/lease revocation, agent-originated receipt/artifact/PhaseOutput, and secret-free package material.
