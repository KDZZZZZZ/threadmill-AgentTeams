...
M5-C3-1 将 Artifact 的 logical metadata、hash→ref mapping 与 TaskID+InvocationID access grant 纳入同一 Runtime SQLite authority。artifact bytes 仍属于外部 blob/object store；publisher 成功但 SQLite metadata transaction 失败会留下可 GC orphan blob，反向绝不提交指向未验证 blob 的 metadata。metadata insert、origin-owner grant 和唯一 `ArtifactRegistered` outbox 在一个 transaction 中完成；相同 content hash 保持既有 first-metadata-wins 去重语义，重复 grant/event 幂等。ExecutionEpoch 仅可作为审计 evidence，绝不进入 logical ownership key。此处仍不是 full event sourcing：snapshot 是 online recovery authority，outbox 仅为 audit/projection/reconciliation input；dispatcher/cursor 不属于 C3-1。
